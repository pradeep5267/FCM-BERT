...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states,audio_data, attention_mask, all_train_visual = None):
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor,audio_data,attention_mask, all_train_visual):
        self_output = self.self(input_tensor,audio_data,attention_mask, all_train_visual)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states,all_audio_data,attention_mask, all_train_visual):
        attention_output = self.attention(hidden_states,all_audio_data,attention_mask, all_train_visual)
...
class BertFinetun(nn.Module):
    def __init__(self, config):
        ...
        # self.proj_v = nn.Conv1d(5,20, kernel_size=1, padding=0, bias=False)
        ...
        self.visual_weight_1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=False)
       
        self.audio_weight_1.data.fill_(1)
        self.text_weight_1.data.fill_(1)
        self.visual_weight_1.fill_(1)
    ...
    def forward(self, hidden_states,pooled_output,audio_data,attention_mask,all_train_visual):
        ...
        all_train_visual = all_train_visual.transpose(1, 2)
        all_train_visual = self.proj_v(all_train_visual)
        all_train_visual = all_train_visual.transpose(1, 2)
        ...
        visual_att = torch.matmul(all_train_visual,all_train_visual.transpose(-1, -2))
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states,all_audio_data,attention_mask, output_all_encoded_layers=True, all_train_visual = None):
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states,all_audio_data, attention_mask, all_train_visual)
...
# input_ids,all_audio_data,token_type_ids, attention_mask, output_all_encoded_layers=True, all_train_visual = all_train_visual
    def forward(self, input_ids,all_audio_data,token_type_ids=None, attention_mask=None,output_all_encoded_layers=True, all_train_visual = None):
        ...
        encoded_layers = self.encoder(embedding_output,
                                      ...
                                      all_train_visual = all_train_visual)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, all_audio_data,token_type_ids=None, attention_mask=None, labels=None,all_train_visual = None):
                                                                    # input_ids,all_audio_data,token_type_ids=None, attention_mask=None,output_all_encoded_layers=True
        encoder_lastoutput, pooled_output, extend_mask = self.bert(input_ids,all_audio_data,token_type_ids, attention_mask, output_all_encoded_layers=True, all_train_visual = all_train_visual)
        pooled_output = self.dropout(pooled_output)
        pooled_output,text_att,fusion_att = self.BertFinetun(encoder_lastoutput,pooled_output,all_audio_data,extend_mask,all_train_visual)
        pooled_output = self.dropout(pooled_output)
