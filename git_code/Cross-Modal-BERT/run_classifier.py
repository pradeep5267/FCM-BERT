...
def main(i):
    ...
    with open('/media/disk_mount/cmu_mosi_git/immortal_cmu_dataset/mosi_data.pkl', 'rb') as fhandle:
        b = pickle.load(fhandle)
    train_visual = b['train']['vision']
    valid_visual = b['valid']['vision']
    test_visual = b['test']['vision']

    logger_mod = logging.getLogger()
    handler_mod = logging.FileHandler('run_classifier_logfile.log')
    logger_mod.addHandler(handler_mod)
    ...
    if args.do_train:
        ...
        all_train_visual = torch.tensor(train_visual, dtype=torch.float32)

        # logger_mod.error(
        #                 all_train_audio.shape,'**********',
        #                 all_input_ids.shape,'**********',
        #                 all_input_mask.shape,'**********',
        #                 all_segment_ids.shape,'**********',
        #                 all_label_ids.shape,'**********',
        #                 all_train_visual.shape,'**********'
        #                 )


        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_train_audio, all_label_ids, all_train_visual)
...
###################################################### make changes above ############################################
        ...
        all_valid_visual = torch.tensor(valid_visual, dtype=torch.float32,requires_grad=True)


        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,all_valid_audio,all_label_ids, all_valid_visual)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, all_train_audio, label_ids, all_train_visual = batch
                # loss = model(input_ids, all_train_audio,segment_ids, input_mask, label_ids)
                # print(f'train_data visual =====> {all_train_visual.shape}')
                # print(f'train_data audio =====> {all_train_audio.shape}')
                # print(f'train_data text =====> {input_mask.shape}')
               
                loss = model(input_ids, all_train_audio,segment_ids, input_mask, label_ids, all_train_visual)
                if n_gpu > 1:
            ...
            print('********************* Start of eval loop *****************************')

            for input_ids, input_mask, segment_ids,all_valid_audio,label_ids,all_valid_visual in tqdm(eval_dataloader, desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                label_ids = label_ids.to(device)
                all_valid_audio = all_valid_audio.to(device)
                all_valid_visual = all_valid_visual.to(device)
                with torch.no_grad():
                    tmp_eval_loss = model(input_ids, all_valid_audio,segment_ids, input_mask,label_ids,all_valid_visual)
                    logits,_,_ = model(input_ids,all_valid_audio, segment_ids, input_mask, all_train_visual = all_valid_visual)
    ...
    if args.do_test:
        ...
        all_test_visual = torch.tensor(test_visual, dtype=torch.float32,requires_grad=True)


        test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,all_test_audio, all_test_visual)
        # Run prediction for full data
        ...
        with torch.no_grad():
            for input_ids, input_mask, segment_ids, label_ids, all_test_audio,all_test_visual in tqdm(test_dataloader, desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                label_ids = label_ids.to(device)
                all_test_audio = all_test_audio.to(device)
                all_test_visual = all_test_visual.to(device)

                with torch.no_grad():
                    tmp_test_loss = model(input_ids, all_test_audio,segment_ids, input_mask, label_ids, all_test_visual)
                    logits,text_attention,fusion_attention = model(input_ids, all_test_audio,segment_ids, input_mask, all_train_visual = all_test_visual)
